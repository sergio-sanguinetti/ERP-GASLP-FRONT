...
export default function FormasPagoPage() {
  ...
  const guardarFormaPago = () => {
    if (tipoDialogo === 'crear') {
      ...
      setFormasPagoList(prev => prev.map(fp => 
        fp.id === formaPagoSeleccionada.id 
          ? { ...fp, ...formulario, fechaModificacion: new Date().toISOString().split('T')[0], usuarioModificacion: 'Usuario Actual' }
          : fp
      ))
    }
    cerrarDialogo()
  ...
  const getTipoIcon = (tipo: string) => {
    switch (tipo) {
      case 'efectivo': return <AttachMoneyIcon />
      case 'terminal': return <CreditCardIcon />
      case 'transferencia': return <AccountBalanceIcon />
      case 'cheque': return <ReceiptIcon />
      case 'deposito': return <AccountBalanceIcon />
      case 'credito': return <PaymentIcon />
  ...
  const getTipoColor = (tipo: string) => {
    switch (tipo) {
      ...
      case 'deposito': return 'secondary'
      case 'credito': return 'error'
  ...
  const formasPagoFiltradas = formasPagoList.filter(fp => {
    ...
    const cumpleActiva = !filtros.activa || (filtros.activa === 'activa' && fp.activa) || (filtros.activa === 'inactiva' && !fp.activa)
  ...
  return (
    <Box sx={{ p: 3 }}>
        ...
        <CardContent>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
            <Typography variant='h6'>
              Formas de Pago ({formasPagoFiltradas.length})
            </Typography>
            <Box sx={{ display: 'flex', gap: 1 }}>
                  ...
                  <TableRow key={formaPago.id} hover>
                    ...
                    <TableCell align='right'>
                      <Typography variant='body2'>
                        {formaPago.comisionPorcentaje}%
                      </Typography>
                    </TableCell>
                    <TableCell align='right'>
                      <Typography variant='body2'>
                        {formaPago.diasLiquidacion} días
                      </Typography>
                    </TableCell>
                    <TableCell align='center'>
                      <Box sx={{ display: 'flex', gap: 0.5 }}>
                        ...
                        <Tooltip title='Eliminar'>
                          <IconButton 
                            size='small' 
                            onClick={() => abrirDialogo('eliminar', formaPago)}
                            color='error'
                          >
                            <DeleteIcon />
                          </IconButton>
      ...
      <Dialog open={dialogoAbierto && (tipoDialogo === 'crear' || tipoDialogo === 'editar')} onClose={cerrarDialogo} maxWidth='md' fullWidth>
        <DialogTitle>
      ...
      <Dialog open={dialogoAbierto && tipoDialogo === 'ver'} onClose={cerrarDialogo} maxWidth='md' fullWidth>
                ...
                <Grid item xs={12}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
                    ...
                    <Box>
                      ...
                      <Chip 
                        label={tiposFormaPago.find(t => t.value === formaPagoSeleccionada.tipo)?.label || formaPagoSeleccionada.tipo}
                      ...
                      <Grid container spacing={2}>
                        ...
                        <Grid item xs={6}>
                          ...
                          <Typography variant='body1'>
                            {formaPagoSeleccionada.configuracion.limiteMaximo ? `$${formaPagoSeleccionada.configuracion.limiteMaximo.toLocaleString()}` : 'Sin límite'}
                          </Typography>
                        </Grid>
                        <Grid item xs={6}>
                          <Typography variant='body2' color='text.secondary'>
                          ...
                          <Typography variant='body1'>
                            {formaPagoSeleccionada.configuracion.limiteMinimo ? `$${formaPagoSeleccionada.configuracion.limiteMinimo.toLocaleString()}` : 'Sin límite'}
                          </Typography>
                        </Grid>
                      </Grid>
                    </CardContent>
                      ...
                      <Grid container spacing={2}>
                        <Grid item xs={6}>
                          <FormControlLabel
                            control={
                              <Checkbox
                                checked={formaPagoSeleccionada.requiereValidacion}
                                disabled
                              />
                            }
                            label='Requiere Validación'
                        ...
                        <Grid item xs={6}>
                          <FormControlLabel
                            control={
                              <Checkbox
                                checked={formaPagoSeleccionada.requiereFolio}
                                disabled
                              />
                            }
                            label='Requiere Folio'
                        ...
                        <Grid item xs={6}>
                          <FormControlLabel
                            control={
                              <Checkbox
                                checked={formaPagoSeleccionada.configuracion.requiereComprobante}
                                disabled
                              />
                            }
                            label='Requiere Comprobante'
                        ...
                        <Grid item xs={6}>
                          <FormControlLabel
                            control={
                              <Checkbox
                                checked={formaPagoSeleccionada.configuracion.permiteCambio}
                                disabled
                              />
                            }
                            label='Permite Cambio'
                    ...
                    <CardContent>
                      ...
                      <Grid container spacing={2}>
                        <Grid item xs={6}>
                          ...
                          <Typography variant='body1'>
                            {formaPagoSeleccionada.usuarioCreacion}
                          </Typography>
                        </Grid>
                        <Grid item xs={6}>
                          <Typography variant='body2' color='text.secondary'>
                        ...
                        <Grid item xs={6}>
                          ...
                          <Typography variant='body1'>
                            {formaPagoSeleccionada.usuarioModificacion}
                          </Typography>
                        </Grid>
                        <Grid item xs={6}>
                          <Typography variant='body2' color='text.secondary'>
        ...
        <DialogActions>
          ...
          <Button onClick={() => abrirDialogo('editar', formaPagoSeleccionada || undefined)} variant='contained' startIcon={<EditIcon />}>
            Editar
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={dialogoAbierto && tipoDialogo === 'eliminar'} onClose={cerrarDialogo}>
        ...
        <DialogContent>
          <DialogContentText>
            ¿Estás seguro de que deseas eliminar la forma de pago "{formaPagoSeleccionada?.nombre}"?
